...
import { Plus, Search, Users, ArrowUpRight, List, LayoutGrid, Building2, Briefcase, UserCheck, Clock, UserX, ChevronLeft, ChevronRight } from "lucide-react";
      ...
      {viewMode === "list" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-start">
          {KANBAN_COLUMNS.map((col) => {
            ...
            const colPage = kanbanPages[col.status] || 1;
            const colTotalPages = Math.ceil(columnEmployees.length / pageSize);
            const paginatedColEmployees = columnEmployees.slice((colPage - 1) * pageSize, colPage * pageSize);
            const Icon = col.icon;
            return (
              <div key={col.status} className="flex flex-col rounded-lg border border-border bg-card p-3 space-y-3 min-h-[300px]">
                ...
                <div className="space-y-2.5 flex-1">
                  {paginatedColEmployees.length === 0 ? (
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-border text-[11px] font-mono text-muted-foreground">
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        className="h-6 w-6"
                        disabled={colPage <= 1}
                        onClick={() => setKanbanPages((prev) => ({ ...prev, [col.status]: colPage - 1 }))}
                      >
