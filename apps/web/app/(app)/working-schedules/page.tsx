...
export default function WorkingSchedulesPage() {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
      <Card className="flex flex-col"><CardHeader className="flex flex-row items-center justify-between space-y-0"><CardTitle className="text-sm">Active schedules</CardTitle><Button size="sm" variant="outline" onClick={reset}><Plus className="mr-1.5 h-3.5 w-3.5" />New schedule</Button></CardHeader><CardContent className="space-y-3 flex-1 flex flex-col justify-between">
        <div className="space-y-2">
          {loading ? <div className="flex items-center gap-2 py-8 text-xs text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Loading schedules...</div> : schedules.length === 0 ? <div className="py-8 text-center text-xs text-muted-foreground">No active schedules found.</div> : paginatedSchedules.map((schedule) => <div key={schedule.id} className="flex items-center justify-between rounded-md border p-3"><div><div className="flex items-center gap-2 text-sm font-medium"><Clock3 className="h-3.5 w-3.5 text-muted-foreground" />{schedule.name}<Badge variant="secondary" className="text-[10px]">{schedule.code ?? "No code"}</Badge></div><p className="mt-1 text-[11px] text-muted-foreground">{schedule.weeklyHours.toFixed(2)} hours/week · {schedule.contractCount} active contract(s)</p><p className="text-[11px] text-muted-foreground">{schedule.description || "No description"}</p></div><div className="flex gap-1"><Button size="icon" variant="ghost" onClick={() => edit(schedule)} title="Edit"><Edit3 className="h-3.5 w-3.5" /></Button><Button size="icon" variant="ghost" onClick={() => void deactivate(schedule.id)} title="Deactivate"><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button></div></div>)}
        </div>
