...
const pendingActions: Array<{ title: string; sub: string; href: string; color: string; Icon: LucideIcon }> = [
  { title: "Attendance exceptions", sub: "7 records need review", href: "/attendance", color: "bg-amber-50 text-amber-700", Icon: CalendarClock },
  { title: "Leave approvals", sub: "3 requests pending", href: "/time-off/requests", color: "bg-violet-50 text-violet-700", Icon: CalendarClock },
  { title: "Payroll validation", sub: "September payrun ready", href: "/payroll/payruns", color: "bg-teal-50 text-teal-700", Icon: CheckCircle2 }
];
  ...
  return (
    <div className="grid gap-7">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.45fr_1fr]">
        ...
        <article className="rounded-2xl border bg-white p-6"><div className="flex items-center justify-between"><div><h3 className="font-semibold text-slate-900">Pending actions</h3><p className="mt-1 text-xs text-slate-400">Items that need your attention</p></div><span className="grid size-8 place-items-center rounded-full bg-rose-50 text-xs font-bold text-rose-600">7</span></div><div className="mt-5 grid gap-3">{pendingActions.map(({ title, sub, href, color, Icon }) => <Link href={href} key={title} className="flex items-center gap-3 rounded-xl border border-slate-100 p-3 hover:bg-slate-50"><span className={`grid size-9 place-items-center rounded-lg ${color}`}><Icon className="size-4" /></span><span className="flex-1"><strong className="block text-xs font-semibold text-slate-700">{title}</strong><small className="text-[11px] text-slate-400">{sub}</small></span><ChevronRight className="size-4 text-slate-300" /></Link>)}</div></article>
      </section>
